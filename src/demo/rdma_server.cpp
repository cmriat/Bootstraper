...
bool RdmaServer::allocateMemory(size_t size) {
    ...
    try {
        ...
        _memory_region->rkey_str = std::string(
            static_cast<char*>(_memory_region->rkey_buffer), rkey_size);
    ...
    _proto.register_handler(1, [this](seastar::sstring client_id) {
        if (!_memory_region) {
            return seastar::make_exception_future<seastar::sstring>(
                std::runtime_error("Memory region not allocated"));
        }
        
        // Return memory address, size, and rkey as a string
        std::string addr_str = std::to_string(reinterpret_cast<uint64_t>(_memory_region->addr));
        std::string size_str = std::to_string(_memory_region->size);
        
        return seastar::make_ready_future<seastar::sstring>(
            addr_str + ":" + size_str + ":" + _memory_region->rkey_str);
    });
    
    // 2. Create UCXX endpoint
    _proto.register_handler(2, [this](seastar::sstring client_addr, uint16_t client_ucxx_port) {
        // Return server's UCXX port for the client to connect to
