...
enum data_type_t {
  DATA_TYPE_0XAA,    // Original 0xaa pattern (default)
  DATA_TYPE_CHAR,    // char type initialized to 0xaa
  DATA_TYPE_EMPTY    // Empty buffer (no initialization)
};

typedef std::unordered_map<transfer_type_t, std::shared_ptr<ucxx::Buffer>> BufferMap;
...
struct app_context_t {
  ...
  int cuda_device              = -1;  // -1 means use current device
  data_type_t data_type        = DATA_TYPE_0XAA;  // Default to 0xaa pattern
};
...
{
  std::cerr << " perftest client/server example with GPU memory support" << std::endl;
  std::cerr << std::endl;
  std::cerr << "Usage: perftest [server-hostname] [options]" << std::endl;
  std::cerr << std::endl;
  ...
  std::cerr << "  -g          use GPU memory (RMM buffers) instead of host memory (disabled)" << std::endl;
  std::cerr << "  -d <int>    CUDA device ID to use (default: current device)" << std::endl;
  std::cerr << "  -t <type>   data type: '0xaa' (default), 'char', 'empty'" << std::endl;
  std::cerr << "              0xaa: fill with 0xaa pattern" << std::endl;
  std::cerr << "              char: char type initialized to 0xaa" << std::endl;
  std::cerr << "              empty: no initialization (empty buffer)" << std::endl;
...
ucs_status_t parseCommand(app_context_t* app_context, int argc, char* const argv[])
{
  optind = 1;
  int c;
  while ((c = getopt(argc, argv, "m:p:s:w:n:d:t:ervgh")) != -1) {
    switch (c) {
      case 'm':
      ...
      case 'n':
        ...
        break;
      case 'd':
        app_context->cuda_device = atoi(optarg);
        if (app_context->cuda_device < 0) {
          std::cerr << "Wrong CUDA device ID: " << app_context->cuda_device << std::endl;
          return UCS_ERR_INVALID_PARAM;
        }
        break;
      case 't':
        if (strcmp(optarg, "0xaa") == 0) {
          app_context->data_type = DATA_TYPE_0XAA;
        } else if (strcmp(optarg, "char") == 0) {
          app_context->data_type = DATA_TYPE_CHAR;
        } else if (strcmp(optarg, "empty") == 0) {
          app_context->data_type = DATA_TYPE_EMPTY;
        } else {
          std::cerr << "Invalid data type: " << optarg << std::endl;
          std::cerr << "Valid options: 0xaa, char, empty" << std::endl;
          return UCS_ERR_INVALID_PARAM;
        }
        break;
      ...
      case 'g': app_context->use_gpu_memory = true; break;
      case 'h':
...
BufferMapPtr allocateTransferBuffers(size_t message_size, bool use_gpu_memory, data_type_t data_type)
{
  std::shared_ptr<ucxx::Buffer> sendBuffer, recvBuffer;

  if (use_gpu_memory) {
#ifdef CUDA_AVAILABLE
    sendBuffer = ucxx::allocateBuffer(ucxx::BufferType::RMM, message_size);
    recvBuffer = ucxx::allocateBuffer(ucxx::BufferType::RMM, message_size);

    // Initialize buffers based on data type
    switch (data_type) {
      case DATA_TYPE_0XAA: {
        // Fill send buffer with 0xaa pattern
        std::vector<char> host_data(message_size, 0xaa);
        cudaError_t err = cudaMemcpy(sendBuffer->data(), host_data.data(),
                                     message_size, cudaMemcpyHostToDevice);
        if (err != cudaSuccess) {
          throw std::runtime_error("Failed to copy 0xaa data to GPU send buffer: " +
                                   std::string(cudaGetErrorString(err)));
        }
        break;
      }
      case DATA_TYPE_CHAR: {
        // Fill send buffer with char type initialized to 0xaa
        std::vector<char> host_data(message_size);
        ...
        cudaError_t err = cudaMemcpy(sendBuffer->data(), host_data.data(),
                                     message_size, cudaMemcpyHostToDevice);
        if (err != cudaSuccess) {
          throw std::runtime_error("Failed to copy char data to GPU send buffer: " +
                                   std::string(cudaGetErrorString(err)));
        }
        break;
      }
      case DATA_TYPE_EMPTY:
        ...
        std::cout << "Using empty GPU buffer (no initialization)" << std::endl;
        break;
    }

    // Initialize recv buffer to zero
    cudaError_t err = cudaMemset(recvBuffer->data(), 0, message_size);
    if (err != cudaSuccess) {
      throw std::runtime_error("Failed to initialize GPU recv buffer: " +
                               std::string(cudaGetErrorString(err)));
    }
#else
    throw std::runtime_error("GPU memory requested but CUDA not available");
#endif
  } else {
    // Use host memory
    sendBuffer = ucxx::allocateBuffer(ucxx::BufferType::Host, message_size);
    recvBuffer = ucxx::allocateBuffer(ucxx::BufferType::Host, message_size);

    // Initialize buffers based on data type
    switch (data_type) {
      case DATA_TYPE_0XAA:
        // Fill send buffer with 0xaa pattern
        memset(sendBuffer->data(), 0xaa, message_size);
        break;
      case DATA_TYPE_CHAR: {
        // Fill send buffer with char type initialized to 0xaa
        char* send_ptr = static_cast<char*>(sendBuffer->data());
        for (size_t i = 0; i < message_size; ++i) {
          send_ptr[i] = static_cast<char>(0xaa);
        }
        break;
      }
      case DATA_TYPE_EMPTY:
        // Empty buffer - no initialization
        std::cout << "Using empty host buffer (no initialization)" << std::endl;
        break;
    }

    // Initialize recv buffer to zero
    memset(recvBuffer->data(), 0, message_size);
  }

  return std::make_shared<BufferMap>(BufferMap{{SEND, sendBuffer}, {RECV, recvBuffer}});
}
                ...
                BufferMapPtr bufferMapReuse)
{
  ...
  if (!app_context.reuse_alloc) localBufferMap = allocateTransferBuffers(app_context.message_size, app_context.use_gpu_memory, app_context.data_type);
  ...
  if (app_context.verify_results && app_context.data_type != DATA_TYPE_EMPTY) {
    if (app_context.use_gpu_memory) {
#ifdef CUDA_AVAILABLE
      // For GPU buffers, we need to copy data to host for verification
      std::vector<char> send_host_data(app_context.message_size);
      std::vector<char> recv_host_data(app_context.message_size);

      cudaError_t err = cudaMemcpy(send_host_data.data(), (*bufferMap)[SEND]->data(),
                                   app_context.message_size, cudaMemcpyDeviceToHost);
      if (err != cudaSuccess) {
        std::cerr << "Failed to copy send buffer from GPU for verification: "
                  << cudaGetErrorString(err) << std::endl;
      } else {
        err = cudaMemcpy(recv_host_data.data(), (*bufferMap)[RECV]->data(),
                         app_context.message_size, cudaMemcpyDeviceToHost);
        if (err != cudaSuccess) {
          std::cerr << "Failed to copy recv buffer from GPU for verification: "
                    << cudaGetErrorString(err) << std::endl;
        } else {
          // Verify data correctness
          for (size_t j = 0; j < app_context.message_size; ++j) {
            assert(recv_host_data[j] == send_host_data[j]);
          }
        }
      }
#endif
    } else {
      // For host buffers, direct comparison
      char* send_ptr = static_cast<char*>((*bufferMap)[SEND]->data());
      char* recv_ptr = static_cast<char*>((*bufferMap)[RECV]->data());
      for (size_t j = 0; j < app_context.message_size; ++j) {
        assert(recv_ptr[j] == send_ptr[j]);
      }
    }
  } else if (app_context.verify_results && app_context.data_type == DATA_TYPE_EMPTY) {
    std::cout << "Skipping verification for empty buffer data type" << std::endl;
  }
...
int main(int argc, char** argv)
{
  ...
  // Print configuration
  std::cout << "Perftest Configuration:" << std::endl;
  std::cout << "  Memory type: " << (app_context.use_gpu_memory ? "GPU (RMM)" : "Host") << std::endl;
  if (app_context.cuda_device >= 0) {
    std::cout << "  CUDA device: " << app_context.cuda_device << std::endl;
  }
  std::cout << "  Data type: ";
  switch (app_context.data_type) {
    case DATA_TYPE_0XAA:
      std::cout << "0xaa pattern";
      break;
    case DATA_TYPE_CHAR:
      std::cout << "char initialized to 0xaa";
      break;
    case DATA_TYPE_EMPTY:
      std::cout << "empty buffer (no initialization)";
      break;
  }
  std::cout << std::endl;
  std::cout << "  Message size: " << app_context.message_size << " bytes" << std::endl;
  std::cout << "  Iterations: " << app_context.n_iter << std::endl;
  std::cout << "  Warmup iterations: " << app_context.warmup_iter << std::endl;
  std::cout << "  Reuse allocation: " << (app_context.reuse_alloc ? "Yes" : "No") << std::endl;
  std::cout << "  Verify results: " << (app_context.verify_results ? "Yes" : "No") << std::endl;
  std::cout << std::endl;
  ...
  if (app_context.use_gpu_memory) {
    ...
    if (err != cudaSuccess || device_count == 0) {
      std::cerr << "Error: GPU memory requested but no CUDA devices available" << std::endl;
      return -1;
    }
    ...
    if (app_context.cuda_device >= 0) {
      if (app_context.cuda_device >= device_count) {
        std::cerr << "Error: CUDA device " << app_context.cuda_device
                  << " not available. Available devices: 0-" << (device_count-1) << std::endl;
        return -1;
      }
      err = cudaSetDevice(app_context.cuda_device);
      if (err != cudaSuccess) {
        std::cerr << "Error: Failed to set CUDA device " << app_context.cuda_device
                  << ": " << cudaGetErrorString(err) << std::endl;
        return -1;
      }
      std::cout << "Set CUDA device to: " << app_context.cuda_device << std::endl;
    }
...
#else
  if (app_context.use_gpu_memory) {
    std::cerr << "Error: GPU memory requested but CUDA not available at compile time" << std::endl;
    return -1;
  }
  if (app_context.cuda_device >= 0) {
    std::cerr << "Error: CUDA device specified but CUDA not available at compile time" << std::endl;
    return -1;
  }
#endif
  ...
  // Allocate wireup buffers (small host buffers for initial handshake)
  const size_t wireup_size = 3 * sizeof(char);
  auto wireupSendBuffer = ucxx::allocateBuffer(ucxx::BufferType::Host, wireup_size);
  auto wireupRecvBuffer = ucxx::allocateBuffer(ucxx::BufferType::Host, wireup_size);

  // Initialize wireup send buffer with test pattern
  char* send_ptr = static_cast<char*>(wireupSendBuffer->data());
  send_ptr[0] = 1; send_ptr[1] = 2; send_ptr[2] = 3;

  auto wireupBufferMap = std::make_shared<BufferMap>(
    BufferMap{{SEND, wireupSendBuffer}, {RECV, wireupRecvBuffer}});
  ...
  char* recv_ptr = static_cast<char*>((*wireupBufferMap)[RECV]->data());
  for (size_t i = 0; i < 3; ++i)
    assert(recv_ptr[i] == send_ptr[i]);

  BufferMapPtr bufferMapReuse;
  if (app_context.reuse_alloc) bufferMapReuse = allocateTransferBuffers(app_context.message_size, app_context.use_gpu_memory, app_context.data_type);
